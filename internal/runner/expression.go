...
func mergeEnvironmentSecrets(rc *RunContext, base map[string]string) map[string]string {
	return mergeEnvironmentMaps(rc, base, func(c *Config) map[string]map[string]string {
		if c == nil {
			return nil
		}
		return c.EnvironmentSecrets
	})
}

func mergeEnvironmentVars(rc *RunContext, base map[string]string) map[string]string {
	return mergeEnvironmentMaps(rc, base, func(c *Config) map[string]map[string]string {
		if c == nil {
			return nil
		}
		return c.EnvironmentVars
	})
}

func mergeEnvironmentMaps(rc *RunContext, base map[string]string, pick func(*Config) map[string]map[string]string) map[string]string {
	if rc == nil || rc.Config == nil || rc.Run == nil {
		return base
	}
	byEnv := pick(rc.Config)
	if len(byEnv) == 0 {
		return base
	}
	job := rc.Run.Job()
...
func getWorkflowVars(_ context.Context, rc *RunContext) map[string]string {
	return mergeEnvironmentVars(rc, rc.Config.Vars)
}
