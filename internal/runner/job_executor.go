...
func printStepSummaries(ctx context.Context, rc *RunContext) {
	...
	for stepID, body := range rc.stepSummaries {
		...
		aggregated.WriteString(stepID)
		aggregated.WriteString("\n\n")
		aggregated.WriteString(body)
		aggregated.WriteString("\n\n")
	}
	if rc.Config != nil && rc.Config.StepSummaryFile != "" && aggregated.Len() > 0 {
		if err := os.WriteFile(rc.Config.StepSummaryFile, []byte(aggregated.String()), 0o644); err != nil {
			logger.Warningf("failed to write step summary file %s: %v", rc.Config.StepSummaryFile, err)
		} else {
			logger.Infof("Wrote step summaries to %s", rc.Config.StepSummaryFile)
		}
	}
}
